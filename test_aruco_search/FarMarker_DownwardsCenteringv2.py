...
def video_thread(controller):
    ...
    while controller.is_running:
        ...
        if ids is not None and rvecs is not None and tvecs is not None:
            ...
            #controller.set_marker_center(None, None)
...
#         return (marker_center_x, marker_center_y), frame
#     return None, frame
    ...
    while controller.is_running:
        ...
        if rvecs is not None and tvecs is not None:
            ...
            if abs(x_offset) < center_tolerance and abs(y_offset) < center_tolerance:
                ...
                lr = int(-delta_y)
                fb = int(-delta_x)
                # lr = int(np.clip(-dx * kx, -30, 30))
                # fb = int(np.clip(-dy * ky, -30, 30))
            ...
            if count < 10:
                ...
                controller.drone.send_rc_control(0,0,20,0)
                time.sleep(0.3)
            time.sleep(1)
            continue
...
def movement_thread(controller):
    ...
    for i in range(num_segments):
        if not controller.is_running:
            break
        if i != 0:
            dis = get_distance_with_retry(controller)
            if dis is not None: 
                forward_per_segment = int(controller.get_distance()/(num_segments-i))
