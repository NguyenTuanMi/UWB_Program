...
import threading
import json
from pathfinding import find_path, draw_path, enlarge
...
safe_area = [(58.59, 155.69), (110.11, 156.19), (110.11, 146.94), (58.39, 146.53)]
...
def streamVideo(drone):
    cv2.namedWindow("Tello Camera", cv2.WINDOW_NORMAL)
    frame_read = drone.frame_read()
    detection = cv2.aruco.ArucoDetector(aruco_dict, parameters)
    while True: 
        ...
        corners, ids, rejected = detection.detectMarkers(frame)
        ...
        if ids is not None:
            ...
            for i in range(0, len(ids)):
                ...
                if checkMarkerCenter(frame_center, marker_center):
                    ...
                    print("Marker's position", pose)
                    #pose = json.dumps(pose).encode()
                    retries = 0
                    path = find_path(pose, mode = 1, safe_y = max(P[1] for P in safe_area) - 2) #This part would be computational heavy (A star shit)
                    P = draw_path(path, get_intermitten = True, draw_intermitten = True, WANT_TO_MOVE = False)
                    P = json.dumps(P).encode()
                    while retries <= 10: #Send at least 10 times to make sure that P will reach the wanted place
                        sock.sendto(P, (broadcast_address, port)) 
                        retries += 1
