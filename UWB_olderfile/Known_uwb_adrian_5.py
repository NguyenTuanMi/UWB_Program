...
print("This is the tag_id", tag_id)
'''
...
def locate_marker(drone, id):
    while True:
        ...
        if dis[id] == 0:
            ...
            marker_corners, marker_IDs, _ = corners, ids, rejected = detector.detectMarkers(frame)
...
def stream_video(drone):
    ...
    while True:
        ...
        if not ret:
            break
        gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        marker_corners, marker_IDs, reject = corners, ids, rejected = detector.detectMarkers(frame)
...
def validate_waypoints():
    ...
    if drone_id in group_1:
        with open('waypoint_5.json', 'r') as f:
            data = json.load(f)
...
def execute_waypoints(drone):
    ...
    try:
        ...
        if drone_id in group_1:
            with open('waypoint_grp1.json', 'r') as f:
                data = json.load(f)
        elif drone_id in group_2:
            with open('waypoint_grp2.json', 'r') as f:
                data = json.load(f)
