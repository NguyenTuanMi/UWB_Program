...
class DroneController:
    ...
    def get_marker_x(self): #Get the target marker
...
def get_distance_with_retry(controller, max_attempts=100):
...
def get_marker_x_with_retry(controller, max_attempts=5):
    for attempt in range(max_attempts):
        d = controller.get_marker_x()
        if d is not None:
            return d
        time.sleep(0.1)
    return None
...
def video_thread(controller):
    ...
    while controller.is_running:
        ...
        if ids is not None and rvecs is not None and tvecs is not None:
            for id in ids:
                if id[0] not in marker_list and id[0] != 0 and id[0] < 20:
                    marker_list.append(id[0])
                    print(f"New marker {id[0]} is detected")
...
def center_on_marker(controller):
    marker_x = get_marker_x_with_retry(controller) 
...
def downward_center_and_land(controller, target_marker_id):
    controller.marker_client.send_update('marker', marker_id=int(target_marker_id), detected=True)
    controller.marker_client.send_update('marker', marker_id=int(target_marker_id), landed=True)
    for marker_id in marker_list:
        if marker_id != target_marker_id:
            controller.marker_client.send_update('marker', marker_id=int(marker_id), detected=False)
...
def scan_marker(controller, marker_client, release = False):
    ...
    if marker_list[0] not in current_detected_markers:
        return 
    locate_marker(controller, marker_list[0])
...
def scan_for_marker(controller, marker_client):
    ...
    # marker_list = controller.get_marker_list()
    # print(f"Current marker list {marker_list}")
    rotation = 0
    while rotation < 360:
        for id in marker_list:
            ...
            if marker_client.is_marker_available(id) and id in controller.valid_ids:
                ...
                locate_marker(controller,id)
            # locate_marker(controller, id)
        controller.drone.rotate_clockwise(60)
        rotation += 60
        time.sleep(1)
...
def locate_marker(controller, id):
    ...
    initial_distance = get_distance_with_retry(controller)
    ...
    for i in range(num_segments):
        if not controller.is_running:
            break
        if i != 0:
            dist = get_distance_with_retry(controller)
            if dist is None: 
                forward_per_segment = initial_segment
            else: 
                forward_per_segment = int(dist/(num_segments-(i)))
    ...
    downward_center_and_land(controller, id)
...
def movement_thread(controller, marker_client):
    ...
    pos = uwb_pos
    #sx, sy = uwb_pos
    #controller.set_start_pose(uwb_pos)
    ...
    # controller.drone.rotate_counter_clockwise(heading)
    # time.sleep(2)
