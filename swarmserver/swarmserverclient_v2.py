...
class MarkerServer:
    def __init__(self, host='0.0.0.0', port=5005, show_waypoints_window=True):
        self.host = host
        self.port = port
        self.marker_timeout = 10
        self.waypoint_timeout = 32 #Release waypoint if occupied for more than 1 minute
        ...
        self.broadcast_interval_s = 0.02
        ...
        self.marker_tree = ttk.Treeview(
            self.root,
            columns=("Marker ID", "Detected", "Landed", "Drone ID", "Bonus Position"),
            show="headings",
        )
        ...
        self.marker_tree.heading("Bonus Position", text="Bonus Position")
    ...
    def trigger_land(self, send_repeat:int = 3): #If I trigger land, broadcast a message type land to every clients to land
        """
    ...
    def trigger_relay_execution(self):
        """Send relay execution signal to registered relay clients."""
        relay_drones = [
            drone_id
            for drone_id, status in self.drone_status.items()
            if status.get("relay", False) and status.get("ready", False)
        ]
        if not relay_drones:
            relay_drones = [
                drone_id
                for drone_id, status in self.drone_status.items()
                if status.get("relay", False)
            ]
    ...
    def update_marker_status(self, message:Dict, addr):
        """Update marker status and timestamp"""
        with self.lock:
            try:
                marker_id = str(message["marker_id"])
                drone_id = message.get("drone_id", 0)
                is_claim_req = message.get("detected") is True
                is_release_req = message.get("detected") is False
                is_landed_reg = message.get("landed") is True 
                success = True
                if marker_id != "-1" and marker_id is not None:
                    ...
                    current_owner = self.marker_status[marker_id].get("drone_id")
                    currently_detected = self.marker_status[marker_id].get("detected", False)
                    currently_landed = self.marker_status[marker_id].get("landed", False)
                    ...
                    if is_claim_req:
                        if currently_landed:
                            if current_owner != drone_id:
                                success = False
                        elif not currently_detected or current_owner == drone_id:
                            # Resource is free, OR this drone is renewing its own claim
                            self.marker_status[marker_id]["detected"] = True
                            self.marker_status[marker_id]["drone_id"] = drone_id
                            self.last_updates[marker_id] = time.time()
                            success = True
                        else:
                            # Resource is already owned by another drone. REJECT.
                            success = False
                    
                    elif is_release_req:
                        if current_owner == drone_id or not currently_detected:
                            # Allow release if it's the owner
                            self.marker_status[marker_id]["detected"] = False
                            success = True
                        else:
                            success = False # Cannot release another drone's claim
                    ...
                    if "bonus detected status" in message:
                        self.bonus_status[marker_id] = message["bonus detected status"]
                        if "bonus position" in message and message["bonus position"] is not None:
                            self.bonus_pose[marker_id] = message["bonus position"]
                    ...
                    if "detected" in message:
                        ...
                        self.broadcast_sock.sendto(ack_msg, addr)
    ...
    def update_waypoint_status(self, message: Dict, addr):
        ...
        with self.lock:
            try:
                ...
                if waypoint_id != "-1" and waypoint_id is not None:
                    ...
                    current_owner = self.waypoints_status[waypoint_id].get("drone_id")
                    currently_occupied = self.waypoints_status[waypoint_id].get("occupied", False)

                    if is_claim_req:
                        if not currently_occupied or current_owner == drone_id:
                            self.waypoints_status[waypoint_id]["occupied"] = True
                            self.waypoints_status[waypoint_id]["drone_id"] = drone_id
                            self.waypoints_status[waypoint_id]["last_occupied_time"] = time.time()
                            success = True
                        else:
                            success = False
                    elif is_release_req:
                        if current_owner == drone_id or not currently_occupied:
                            self.waypoints_status[waypoint_id]["occupied"] = False
                            success = True
                        else:
                            success = False

                    # --- NEW: Send ACK directly back to the requesting drone ---
                    if "detected" in message:
                        ack_msg = json.dumps({
                            "type": "waypoint_ack",
    ...
    def update_drone_status(self, message):
        ...
        relay_flag = bool(message.get("relay", self.drone_status[drone_id].get("relay", False)))
        ...
        if message["type"] == "takeoff_request":
            ...
            self.register_ready_drone(drone_id, waiting_list, status, relay=relay_flag)

        elif message["type"] == "status":
            logging.debug(f"{drone_id} status update. Message: {message}")
            self.drone_status[drone_id]["status"] = message["status"]
            self.drone_status[drone_id]["relay"] = relay_flag

        logging.info(f"Drone Statuses: {self.drone_status}")

    def register_ready_drone(self, drone_id, waiting_list:List, status_str:str = None, relay:bool = False):
        """
        ...
        with self.lock:
            ...
            logging.debug(f"The takeoff_waitlist: {self.takeoff_waitlist}")
            ...
            try:
                ...
                try:
                    ...
                    if message.get("type") in ("takeoff_request", "status"):
                        ...
                        self.update_marker_status(message, addr)
                    elif message.get("type") == 'waypoint':
                        self.update_waypoint_status(message, addr)
                    else:
                        logging.warning("Invalid message format received. See handle_messages in swarmserverclient")
                        if addr not in self.clients:
                            with self.lock:
            ...
            for client_addr in self.relay_clients:
                for _ in range(send_repeat):  # Send the message N times for reliability
                    try:
                        ...
                        dead_relay_clients.add(client_addr)

                    logging.debug(f"MarkerServer sent {takeoff_message} to {client_addr} (sent {send_repeat} times for reliability)")

            self.relay_clients -= dead_relay_clients
            if dead_relay_clients:
                logging.info(f"Removed dead relay clients: {dead_relay_clients}")
    ...
    def adjust_column_widths(self):
        ...
        self.marker_tree.column("Bonus Position", width=int(total_width * dec1))
        ...
        try:
            ...
            for marker_id, status in self.marker_status.items():
                if isinstance(status, dict):
                    ...
                    bonus_pose = self.bonus_pose.get(marker_id, "None")
                    self.marker_tree.insert("", "end", values=(marker_id, detected_str, landed_str, drone_id, bonus_pose))
...
class MarkerClient:
    ...
    def send_update(self,  #This function is called periodically to send the update of the client
        ...
        """
        message:Dict = {"drone_id": self.drone_id, "relay": self.relay_status}       # initializes the message with drone_id (client's class attribute)

        requires_ack = (update_type in ["marker", "waypoint"]) and (detected is not None)
        req_key = (update_type, str(marker_id))
        ...
        if update_type == "marker" and marker_id is not None:
            ...
            if bonus_detected is not None:
                message["bonus detected status"] = bonus_detected
                if bonus_position is not None:
                    message["bonus position"] = bonus_position
        ...
        if requires_ack:
            ...
            with self.claims_lock:
                self.pending_claims[req_key] = {"event": event, "success": False}     
        ...
        # --- NEW: Wait for Server ACK ---
        if requires_ack:
            got_ack = event.wait(timeout)
            with self.claims_lock:
                result = self.pending_claims.pop(req_key, {"success": False})
            
            if got_ack and result["success"]:
                logging.info(f"Successfully claimed/updated {update_type} {marker_id}")
                return True # Claim Won
            else:
                logging.warning(f"Failed to claim {update_type} {marker_id} - Server NACK or Timeout")
                return False # Claim Lost (another drone has it)
        
        return True # For status updates that don't need ACKs
    ...
    def receive_updates(self):  # Background thread
        while True:
            try:
                ...
                if message.get("type") in ["marker_ack", "waypoint_ack"]:
                    req_type = "marker" if message.get("type") == "marker_ack" else "waypoint"
                    marker_id = str(message.get("marker_id"))
                    success = message.get("success", False)
                    
                    req_key = (req_type, marker_id)
                    with self.claims_lock:
                        if req_key in self.pending_claims:
                            self.pending_claims[req_key]["success"] = success
                            self.pending_claims[req_key]["event"].set() # Unblock send_update
                
                elif message.get("type") == "takeoff" and self.drone_id in message.get("takeoff_list"):
