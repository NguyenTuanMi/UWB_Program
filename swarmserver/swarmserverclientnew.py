...
class MarkerServer:
    ...
    def check_timeouts(self):
        ...
        while True:
            try:
                ...
                with self.lock:
                    # Check for marker timeouts (why i need to check for marker timeout? Is it because the users only allows a certain amount of time for drones to land on the markers)
                    for marker_id in list(self.marker_status.keys()):
                        last_update = self.last_updates.get(marker_id, 0) #The latest update time of the specific marker id
                        if current_time - last_update > self.marker_timeout:
                            #Why I need to check whether marker_id is in self.marker_status, I thought it is derived from the marker_status list
                    ...
                    # Check for waypoint timeouts (1 minute timeout) (Understandable, wait for 1 miniutes for the drone to move, after 1 minute, allow the other drone to move to this waypoints)
                    ...
                    #Can implement this to check if all bonus victims are all landed and also when to initiate the relay task when all drones are down
                    ...
                    # Reset all landed flags if all valid markers have landed (Why we need to reset the marker when all marker are landed?)
        ...
        with self.lock:
            try:
                ...
                if marker_id != "-1" and marker_id is not None:
                    ...
                    #The format of the marker status for each marker
                    #{
                    #"detected": False,
                    #"landed": False,
                    #"dron_id": 1
        ...
        with self.lock:
            try:
                ...
                if waypoint_id != "-1" and waypoint_id is not None:
                    ...
                    #The format of the waypoint status for each waypoint
                    #{
                    #"occupied": False,
                    #"last_occupied_time": 123,
                    #"drone_id": 1
                    #}
    ...
    def update_drone_status(self, message, address=None):
        """
        ...
        if message["type"] == "takeoff_request":
            ...
            status = message.get("status", None) #Get the status message of the drones to put it on the UI
            if message["relay"] and address is not None:   
                if address not in self.relay_clients:
                    self.relay_clients.add(address)
                    logging.info(f"New relay client connected: {address}")
    ...
    def register_ready_drone(self, drone_id, waiting_list:List, status_str:str = None):
        ...
        with self.lock:
            ...
            all_waiting_drones = set(waiting_list) | {drone_id} #Union of 2 sets, waiting_list and drone_id

            logging.debug(f"all_waiting_drones: {all_waiting_drones}")

            # Check this maybe it will gives a hint to pursue the relay tasks
                ...
                try:
                    ...
                    if message != prev_message:
                        ...
                        if message.get("marker_id") == -1:
                            ...
                            self.update_drone_status(message, address=addr)
                            ...
                            logging.warning("Invalid message format received. See handle_messages in swarmserverclient") #Probably this is the cause of error
                            if addr not in self.clients:
                                with self.lock:
                                    self.clients.add(addr)
                                    logging.info(f"New client connected: {addr}")
...
class MarkerClient:
    def __init__(self, drone_id=0, server_port=5005, broadcast_ip="255.255.255.255", land_callback=None, relay_task=False):
        ...
        self.relay_mission = relay_task #To check if this drone participates in the relay tasks
    ...
    def wait_for_relay(self, drones_list:list, status_message:str = None):
        while not self.relay_triggered:      # This is a holding pattern, which releases upon takeoff_signal being set by the server.
            logging.debug(f"Relay tello {self.drone_id} hovers and wait for relay signal. relay_signal: {self.relay_triggered}")

        logging.info(f"Relay tello {self.drone_id} joins the relay tasks!")
    ...
    def receive_updates(self):  # Background thread
        while True:
            try:
                ...
                elif message.get("type") == "relay_init":
                    logging.info(f"Receive permission to initiate relay task")
