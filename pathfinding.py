    ...
    for i in range(len(nodes)):
        for j in range(len(nodes[i])):
            ...
            if dist < first_search_radius:
                ...
                #The format of state: cost function, path cost, position, parent index, previous direction, direction state
    visited = dict()
    parent = dict()
    while True:
...
def draw_path(**kwargs):
    ...
    for i, p in enumerate(path):
        ...
        if (p[-1] != p[-2]) or (i == len(path)-1):
            ...
            if i != 1 and kwargs["WANT_TO_MOVE"]: 
                drone = kwargs.get("drone")
                print("Move to", (nx, ny))
                execute_waypoints(drone, (nx, ny))
                time.sleep(1)
